...
const Sidet = async () => {
  ...
  return (
    <div className='md:mt-60 mt-120 md:ml-45  ml-0  '>
  ...
  <div key={post.id ? `post-${post.id}` : `post-${index}`} className="w-full mb-3">
    <Link href={`/inside/${post.id ?? index}`} className="block">
      <div className="rounded-md transition flex gap-2">
        ...
        />
        <div>
          <h3 className="bg-[#191B1F] w-11 text-center px-0 rounded-sm py-1 mb-5 text-[12px] text-[#9932CC] font-[poppins] font-semibold">
            {post.title}
          </h3>
          <p className="text-white font-[Poppins] text-sm hover:text-[#9932CC] -mt-5 w-40">
            {post.description}
          </p>
        </div>
      </div>
