...
const Read = async () => {
  ...
  return (
    <div className="justify-center flex md:h-[250vh] h-[340vh] bg-[#191B1F] gap-5  w-full md:w-[130%] mx-auto flex-wrap  -z-30">
      {/* Left section with playstations */}
      <div className="bg-[#232428] w-full h-[347vh] md:h-[230vh] md:ml-34 ml-0 -mt-80 rounded-md z-10 md:mx-0">
            ...
            playstations.map((playstation: Playstation) => (
              <div key={playstation.id} className="w-full mb-3">
                <Link href={`/inside/${playstation.id}`} className="block">
                  <div className="rounded-md transition md:flex   gap-4">
                    <Image
                      src={playstation.imageurl}
                      alt={playstation.title}
                      width={250}
                      height={0}
                      className="rounded-md mb-2 object-cover h-70 w-300  md:w-250 "
                    />
                    <div className="flex flex-col gap-4">
                      <h3 className="bg-[#191B1F] w-20 text-center px-0 rounded-sm py-1 mb-5 text-[12px] text-[#3D55EF] font-[poppins] font-semibold">
                        {playstation.title}
                      </h3>
                      <p className="text-white font-[Poppins] md:text-2xl text-xl w-80   hover:text-[#0000FF] -mt-5 md:w-80">
