...
const Sidet = async () => {
  ...
  return (
    <div className='md:mt-60 mt-100 md:ml-45  ml-0  '>
  ...
  <div key={post.id ? `post-${post.id}` : `post-${index}`} className="w-full mb-3">
    <Link href={`/inside/${post.id ?? index}`} className="block">
      <div className="rounded-md transition flex gap-2">
        ...
        />
        <div>
          <h3 className="bg-[#191B1F] w-11 text-center px-0 rounded-sm py-1 mb-5 text-[12px] text-[#0000FF] font-[poppins] font-semibold">
            {post.title}
          </h3>
